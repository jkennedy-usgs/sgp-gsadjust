...
GSadjust objects for ChannelList: Holds lists of observed data values (g, tilt, temp,
etc.). The lists are copied to ObsTreeStation objects.
...
class ChannelList:
    """Object to stores relative-gravity data as lists.

    Inherited by station object.
    Each item in each list (line, station, etc.) is a sample (i.e., single reading) from
    a relative gravimeter. See __init__ statement for a description of each attribute.
    ...
    def __init__(self):
        ...
        # A station gravity value (.grav or .gmean) is calculated "on the fly" from
    ...
    def __iter__(self):
        ...
        -------
        tuple
            non-empty field in the ChannelList object:
    ...
    def extract_subset_idx(self, start_idx, end_idx):
        ...
        ----------
        start_idx : int
            list index of first value to return.
        end_idx : int
            list index of last value to return.
