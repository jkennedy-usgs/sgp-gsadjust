"""
get_nwis_data Retrieve groundwater-level data from the USGS National Water Information
System.
...
def plot_wells(cross_ref_file, site_IDs):
    ...
    ax.invert_yaxis()
    ax.set_ylabel(
        "Depth to groundwater (meters)", fontname="Times New Roman", fontsize=12
    )
    for tick in ax.get_xticklabels():
        tick.set_fontname("Times New Roman")
    for tick in ax.get_yticklabels():
    ...
    plt.setp(L.texts, family="Times New Roman")
    plt.xlim(
        [datetime.datetime(2006, 1, 1, 0, 0, 0), datetime.datetime(2019, 1, 1, 0, 0, 0)]
    )
    plt.show()
    input()
        ...
        with open(cross_ref_file, "r") as fid:
            for line in fid:
                ...
                if grav_ID.upper() == gravity_station_ID.upper():
                    if len(nwis_ID) != 15:
                        print(
                            "Invalid 15-digit ID for site {}".format(gravity_station_ID)
                        )
                        return 0
                    else:
                        break
                ...
                print(
                    "Gravity Site-ID {} not found in cross-ref file".format(
                        gravity_station_ID
                    )
                )
                return 0
...
def nwis_get_data(nwis_ID):
    ...
    """
    out_dic = dict()
    # rdb_meas retrieval is preferred, it returns both discrete and continuous m
    # easurements.
    today = datetime.datetime.today().strftime("%Y-%m-%d")
    nwis_URL = (
        "http://nwis.waterdata.usgs.gov/nwis/dv?cb_72019=on&format=rdb_meas"
        + f"&site_no={nwis_ID}"
    ...
    if len(d_x) == 0:
        ...
        nwis_URL: str = (
            f"https://nwis.waterdata.usgs.gov/nwis/gwlevels/?site_no={nwis_ID}"
            + "&format=rdb_meas"
        )
        r = requests.get(nwis_URL)
        nwis_data = r.text.split("\n")
...
def parse_discrete_data(nwis_data):
    ...
    for nwis_line in nwis_data:
        ...
        # Need to test for null strings because it's possible for there to be a date
        # without a measurement.
        try:  # parsing by '\t' barely works with the fixed-width fields
            # The parameter we want, 72019_00003 (mean), isn't always in the same
            # column, it depends on what other parameters are output. Search for it here
            if line_elems[0] == "agency_cd":
                gwl_disc_index = [
                    idx for idx, s in enumerate(line_elems) if s == "lev_va"
                ][0]
                continue
                ...
                continue
            elif line_elems[0] == "USGS":  # Doesn't work for other agency codes
                if line_elems[gwl_disc_index] != "":
...
def parse_continuous_data(nwis_data):
    ...
    for nwis_line in nwis_data:
        ...
        try:  # parsing by '\t' barely works with the fixed-width fields
            ...
            if line_elems[0] == "agency_cd":
                gwl_cont_index = [
                    idx for idx, s in enumerate(line_elems) if "72019_00002" in s
                ][0]
                gwl_disc_index = [
                    idx for idx, s in enumerate(line_elems) if "72019_00011" in s
                ][0]
            elif line_elems[0] == "#" and line_elems[1] == "USGS":
                if len(line_elems) > 3:
                ...
                elif line_elems[gwl_disc_index] != "":  # Get depth to water
                    discrete_x.append(parser.parse(line_elems[2]))
                    discrete_y.append(float(line_elems[gwl_disc_index]))
                elif line_elems[gwl_cont_index] != "":
...
def search_nwis(coords):
    ...
    try:
        ...
        for nwis_line in nwis_data:
            ...
            try:  # parsing by '\t' barely works with the fixed-width fields
                if (
                    line_elems[0] == "USGS" and line_elems[13] == "72019"
                ):  # Doesn't work for other agency codes
                    lat = float(line_elems[4])
                    lon = float(line_elems[5])
                    distance = calc_distance(lat, lon, coords[1], coords[0])
                    depth = get_site_property(int(line_elems[1]), prop="wellDepth")
                    ID_dict[line_elems[1]] = [
                        line_elems[2],
                        f"{distance:.0f}",
                        line_elems[21],
                        line_elems[22],
...
def get_gwls_for_date_and_coords(date, coords, buffer=365):
    ...
    # Get DVs
    url = (
        "https://waterservices.usgs.gov/nwis/dv/?format=rdb&bBox="
    ...
    # get gwlevels
    url = (
        r"https://waterservices.usgs.gov/nwis/gwlevels/?format=rdb&bBox="
        + f"{coords[0]:0.3f},{coords[2]:.3f},{coords[1]:.3f},{coords[3]:.3f}"
    ...
    for line in s:
        if line[:4] == "USGS":
            ...
            data.append(
                [line_elems[0], line_elems[1], line_elems[3], line_elems[6], "A"]
            )
...
def get_gwl_change(dates, coords, convert=False, **kwargs):
    ...
    for station in stations:
        ...
        try:
            ...
            if convert:
                out[station] = (wl1 - wl2) * 0.3048
            else:
                out[station] = wl1 - wl2
            lat[station] = (
                float(station[:2])
                + float(station[2:4]) / 60
                + float(station[4:6]) / 3600
            )
            lon[station] = -1 * (
                float(station[6:9])
                + float(station[9:11]) / 60
                + float(station[11:13]) / 3600
            )
            date1[station] = d1
            date2[station] = d2
        except:
...
def get_site_property(ID, prop="monitoringLocationAltitudeLandSurface"):
    ...
    try:
        geo_json = requests.get(
            "/".join([usgs_endpoint, "monitoring-locations", "items", f"USGS-{ID}"])
        )
