...
def use_tide_time_series(MainProg):
    ...
    tides = TimeSeries()
    if (
        fname[len(fname) - 4 : len(fname)] == ".tsf"
        or fname[len(fname) - 4 : len(fname)] == ".TSF"
    ):
        # tsoftfile
...
def use_predict_tides():
    ...
    enterCoordinates.btn1.clicked.connect(
        lambda: self.launchPredict(enterCoordinates, option=1)
    )
    enterCoordinates.btn2 = QtWidgets.QPushButton('Load tidal parameters file', self)
    enterCoordinates.btn2.clicked.connect(
        lambda: self.launchPredict(enterCoordinates, option=2)
    )
...
def launch_predict(self, cwin, option):
    ...
    predict_input_file.write(
        "SAMPLERATE=     60         #sampling interval in seconds \n"
    )
    predict_input_file.write(
        "STATLATITU=   %f       #stations latitude  in degree\n" % self.lat
    )
    predict_input_file.write(
        "STATLONITU=    %f       #stations longitude in degree\n" % self.lon
    )
    predict_input_file.write(
        "STATELEVAT=  %f         #stations elevation in meter\n" % self.alt
    )
    ...
    predict_input_file.write(
        "POLETIDCOR=      0        #ANALYZE parameter for pole corrections (1=yes)\n"
    )
    predict_input_file.write(
        "LODTIDECOR=      0        #ANALYZE parameter for LOD corrections (1=yes)\n"
    )
    ...
    if option == 2:
        ...
        fname, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Load tidal parameters file', self.settings.value('tide_path')
        )
    ...
    shutil.copyfile(
        "./eterna_files/predict.exe", self.output_root_dir + os.sep + "predict.exe"
    )
...
def tide_correction_ts(campaigndata, tide_in):
    ...
    for keysurv, surv in campaigndata.survey_dic.items():
        for keyloop, loop in campaigndata.survey_dic[keysurv].loop_dic.items():
            for keysta, sta in (
                campaigndata.survey_dic[keysurv].loop_dic[keyloop].station_dic.items()
            ):
                tides = (
                    np.round(
                        np.array(
                            [
                                ts_tide_corr(tide_in, t)
                ...
                campaigndata.survey_dic[keysurv].loop_dic[keyloop].station_dic[
                    keysta
                ].grav = [
                    campaigndata.survey_dic[keysurv]
                    .loop_dic[keyloop]
                    .station_dic[keysta]
                    .grav[i]
                    - campaigndata.survey_dic[keysurv]
