...
#
#     gravity_change_map = GravityChangeMap(
#         table, header, coords, full_table, full_header
#     )
#
#
...
#     assert (
#         np.abs(change_plot.figure.axes[0].get_children()[0].get_ydata()[1] + 6.8) \
#         < 0.01
#     )
#
...
#     win.plot()
#     # TODO: This test problem only was two surveys, i.e., 1 increment of gravity
#     # change. Can only do minimal testing, can't test trend.
...
def test_loop_options(qtbot, mainprog):
    ...
    for loop in loops:
        ...
        for i in range(loop.rowCount()):
            ...
            obstreestation.meter = [options_dialog.meter_edit.text()] * len(
                obstreestation.meter
            )
            obstreestation.oper = [options_dialog.operator_edit.text()] * len(
                obstreestation.oper
            )
...
def test_adjustmentoptions_dialog(qtbot, mainprog):
    ...
    assert not ao.cal_coeff_table.isEnabled()
...
def test_selectabsg(qtbot, mainprog):
    sa = SelectAbsg(
        os.path.join("..", "test_data", "field", "Absolute"), parent=mainprog
    )
    sa.show()
